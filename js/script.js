...
const displayPictures = (list) => {
const pictureContainer = document.getElementById("pictures-container");
  const allImages = list.map((item) => {
    return `<div class="image">
      <img src=${item.images[0]} alt=${item.name} class="picture-image">
        <div class="image-info">
          <p id="title">${item.title}</p>
          <p id="name">${item.name}</p>
          <a id="see-more" href="/html/details.html?id=${item.id}">see more</a>
      </div>
   </div>`
  ...
  pictureContainer.innerHTML = allImages;

}

window.onload = function () {
  displayPictures(imageSets);
}

/*Dark button Toggle*/
const darkButton = document.querySelector("#dark-mode-tab");

const toggleDark = () => {
  const body = document.querySelector("body");
  const h1 = document.querySelector("#h1Sub");
  ...
  if (body.style.backgroundColor == "--white-text"){
      darkButton.innerHTML = "Dark Mode";
      h1.style.color = "--black-text";
      title.style.color = "--black-text";
      name.style.color = "--black-text";

  } else {
      darkButton.innerHTML = "Light Mode";
      h1.style.color = "--white-text";
      title.style.color = "--white-text";
      name.style.color = "--white-text";
  }
...
<img src=${images[0]} alt=${name} class="picture-image">
  <div class="image-info">
    <p id="title">${title}</p>
    <p id="name">${name}</p>
    <a id="see-more" href="/html/details.html?id=${id}">see more</a>
</div>
</div>`;
